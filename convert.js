...
function convertHtmlToJsx(html) {
  ...
  // 3. Self closing tags
  content = content.replace(/<img([^>]*?)(?:\s*\/)?>/gi, (match, p1) => `<img${p1} />`);
  content = content.replace(/<input([^>]*?)(?:\s*\/)?>/gi, (match, p1) => `<input${p1} />`);
  content = content.replace(/<br(?:\s*\/)?>/gi, '<br />');
  content = content.replace(/<hr(?:\s*\/)?>/gi, '<hr />');

  // 4. Inline styles
  content = content.replace(/style="([^"]*)"/g, (match, p1) => `style=${processStyle(p1)}`);
